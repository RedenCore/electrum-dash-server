How to run your own Electrum Reden server
========================================
...
Electrum Reden server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/redenpay/electrum-reden-server/blob/master/HOWTO.md
...
document, we assume that user is called 'reden'). We also assume the
reden user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run redend and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit redend 
...
only leaving spare cycles for redend. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running redend and Electrum Reden server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `redend` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser reden --disabled-password
    $ sudo apt-get install git
    $ sudo su - reden
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/reden/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download redend

Recommend downloading latest version directly from Reden.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - reden
    $ cd ~/src && wget https://github.com/RedenCore/Reden/releases/download/1.0.2/reden-1.0.2-ubuntu16.tar.gz
    $ md5sum reden-1.0.2-ubuntu16.tar.gz | grep b69cf160feb39cf3edf53ecb7abf1720
    $ tar xfz reden-1.0.2-ubuntu16.tar.gz
    $ cd reden-1.0.2-ubuntu16/reden
    $ cp -a redend reden-cli reden-tx ~/bin

### Step 3. Configure and start redend

In order to allow Electrum Reden to "talk" to `redend`, we need to set up an RPC
username and password for `redend`. We will then start `redend` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.reden
    $ $EDITOR ~/.reden/reden.conf

Write this in `reden.conf`:
...
If you have an existing installation of redend and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ redend -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `redend`:

    $ redend

Allow some time to pass for `redend` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ reden-cli getblockchaininfo

Before starting the Electrum Reden server your redend should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start redend at boot
time, running as the 'reden' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Reden server
...
### Optional Step 5: Install Electrum Reden dependencies manually

Electrum Reden server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum Reden server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Reden full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Reden server config.
...
### Step 9. Configure Electrum Reden server

Electrum Reden reads a config file (/etc/electrum-reden.conf) when starting up. This
file includes the database setup, redend RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-reden.conf
...
Electrum Reden server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-reden-server.
If you're using the user `reden` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "reden hard nofile 65536" >> /etc/security/limits.conf
     echo "reden soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Reden server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u reden -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close redend for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting redend (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Reden server

The magic moment has come: you can now start your Electrum Reden server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Reden server
...
If you want to stop Electrum Reden server, use the 'stop' command:
...
### Step 12. Test the Electrum Reden server
...
current server and connect to your new Electrum Reden server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
redens to confirm that everything is working properly.
...
If you're operating a public Electrum Reden server please subscribe
